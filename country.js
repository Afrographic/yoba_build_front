...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      await DB.delete_data("delete from country where country_id=:country_id", {
        country_id,
      });
      res.sendStatus(200);
    } catch (error) {
  ...
  async (req, res) => {
    try {
      ...
      let insert = await DB.post_data(
        "insert into country(country_name,code,monnaie) values(:country_name,:code,:monnaie)",
        { country_name, code, monnaie },
      );
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      if (type == "Ville") {
        await DB.update_data(
          "update city set city_name=:name where city_id=:id",
          { name, id },
        );
      }
      res.sendStatus(200);
