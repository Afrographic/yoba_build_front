...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      //Increase view count
      await DB.post_data(
        "insert into vue(offre_id,user_id) values(:offre_id,:user_id)",
        {
          offre_id,
          user_id,
        },
...
//Search for pinned offer
router.get(
  "/pinned-offre-user/:user_id",
  Security.authenticateToken,
  async (req, res) => {
    try {
      let user_id = parseInt(req.params.user_id);
      let offres = await DB.get_data(
        `select * from offre where pinned=1 and user_id=${user_id}`,
        {},
      );
      //Get images
      offres = await Offre_Controller.getOffreMetadatas(user_id, offres);
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      let offre_id = parseInt(req.params.offre_id);
      await DB.update_data(
        "update offre set pinned=1 where offre_id=:offre_id and user_id=:user_id",
        { offre_id, user_id },
      );
      res.sendStatus(200);
    } catch (error) {
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      await DB.update_data(
        "update offre set pinned=0 where offre_id=:offre_id and user_id=:user_id",
        { offre_id, user_id },
      );
      res.sendStatus(200);
    } catch (error) {
