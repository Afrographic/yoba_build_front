...
router.get("/offre/:offset", Security.authenticateToken, async (req, res) => {
  try {
    ...
    //Get images
    for (let i = 0; i <= offres.length - 1; i++) {
      offres[i].images = await Offre_Controller.get_offre_images(
        offres[i].offre_id,
      );
      offres[i].monnaie = await User_Controller.get_user_monnaie(
        offres[i].user_id,
      );
    }
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      let offre = offres[0];
      for (let i = 0; i <= offres.length - 1; i++) {
        let user_id = offres[i].user_id;
        offres[i].images = await Offre_Controller.get_offre_images(
          offres[i].offre_id,
        );
        offres[i].address = await User_Controller.get_address(user_id);
        offres[i].user_info = await User_Controller.get_basic_info(user_id);
        offres[i].monnaie = await User_Controller.get_user_monnaie(
          offres[i].user_id,
        );
      }
...
router.post("/add_offre", Security.authenticateToken, async (req, res) => {
  try {
    ...
    let insert = await DB.post_data(
      "insert into offre(city_id,region_id,country_id,offre_description,user_id,offre_prix,offre_titre,created_at,type,categorie_sous_id) values(:city_id,:region_id,:country_id,:offre_description,:user_id,:offre_prix,:offre_titre,:created_at,:type,:categorie_sous_id)",
      {
        city_id,
        region_id,
        country_id,
    ...
    for (const file_s in req.files) {
      ...
      file.mv(
        `./public/backend_files/bd_${offre_id}${HelperFunction.replace_space_with_underscore(file.name)}`,
      );
      let url = `${Consts.backend_host}/backend_files/bd_${offre_id}${HelperFunction.replace_space_with_underscore(file.name)}`;
      await DB.post_data(
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      let type = "Client";
      let offres = await DB.get_data(
        "select * from offre where type=:type and region_id=:region_id limit 100",
        { type, region_id },
      );
      for (let i = 0; i <= offres.length - 1; i++) {
        offres[i].images = await Offre_Controller.get_offre_images(
