...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (password != user_data[0].user_password) return res.sendStatus(400);
      ...
      //Notify Admins
      User_Controller.notifyAdminsForNewRetrait(user_id, amount);
      res.sendStatus(200);
    } catch (error) {
      serverError(error, res);
    }
  ...
  async (req, res) => {
    try {
      ...
      Notification_Controller.new_notif(
        user_data.user_id,
        `Votre demande de retrait de ${HelperFunction.format_number(retrait_item.amount)}F a ete rejete.\n -${message}`,
      );
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      Notification_Controller.new_notif(
        user_data.user_id,
        `Votre demande de retrait de ${HelperFunction.format_number(retrait_item.amount)}F a ete traite. veuillez consulter votre compte de retrait.`,
      );
      EmailService.send_email(
        user_data.user_email,
...
router.post("/demande_depot", Security.authenticateToken, async (req, res) => {
  try {
    ...
    //Notify Admins
    User_Controller.notifyAdminsForNewRecharge(user_id, montant);
    res.sendStatus(200);
  } catch (error) {
    serverError(error, res);
  }
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      let user_id = depots[0].user_id;
      let message = `Votre recharge de ${HelperFunction.format_number(montant)} F a ete rejetee \n -${raison}`;
      Notification_Controller.new_notif(user_id, message);
      let user_data = await User_Controller.get_user_data(user_id);
      EmailService.send_email(
        user_data.user_email,
...
router.get("/history_depot", Security.authenticateToken, async (req, res) => {
  try {
    ...
    for (let i = 0; i <= depots.length - 1; i++) {
      ...
      if (depots[i].user_info != undefined) {
        response.push(depots[i]);
      }
    }
    res.send(response);
  } catch (error) {
...
router.get(
  ...
  async (req, res) => {
    try {
      let user_id = req.body.user_data.user_id;
      let depots = await DB.get_data(
        "select * from history_depot where user_id=:user_id order by id DESC limit 400",
        {
          user_id: user_id,
        },
      );
...
//get history retrait for a user
router.get(
  "/history_retrait/me",
  Security.authenticateToken,
  async (req, res) => {
    try {
      let user_id = req.body.user_data.user_id;
      let retraits = await DB.get_data(
        "select * from history_retrait where user_id=:user_id order by id DESC limit 400",
        {
          user_id: user_id,
        },
      );
...
router.get("/history_retrait", Security.authenticateToken, async (req, res) => {
  try {
    ...
    for (let i = 0; i <= retraits.length - 1; i++) {
      ...
      if (retraits[i].user_info != undefined) {
        response.push(retraits[i]);
      }
    }
    res.send(response);
  } catch (error) {
