...
router.post("/avis", [Security.authenticateToken], async (req, res) => {
  try {
    ...
    const { user_id, avis, star } = req.body;
    ...
    await DB.delete_data(
      "delete  from avis where user_id=:user_id and from_user_id=:from_user_id",
...
router.get("/avis/:user_id", [Security.authenticateToken], async (req, res) => {
  try {
    ...
    for (let i = 0; i <= rows.length - 1; i++) {
      const user = await User_Controller.get_basic_info(rows[i].from_user_id);
      rows[i].from_user = user;
    }
