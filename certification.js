...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      let { certification, annee, certification_url } = req.body;
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      if (data[0].certification_url != certification_url) {
        if (data[0].certification_url != null) {
          HelperFile.deleteFileFromServer(data[0].certification_url);
        }
      }
