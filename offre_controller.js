...
const { Vue_Controller } = require("./vue_controller");
...
class Offre_Controller {
  ...
  static async get_offre_data(user_id, offre) {
    ...
    offre.total_vue = await Vue_Controller.count_vue(offre.offre_id);
    offre.total_share = await this.get_total_share(offre.offre_id);
    offre.total_like = await this.get_total_liked(offre.offre_id);
    return offre;
  }

  static async get_total_share(offre_id) {
    let data = await DB.get_data(
      "select count(*) as total from share where offre_id=:offre_id",
      { offre_id },
    );
    return data[0].total;
  }
  
  static async get_total_liked(offre_id) {
    let data = await DB.get_data(
      "select count(*) as total from favoris where offre_id=:offre_id",
      { offre_id },
    );
    return data[0].total;
  }
