...
router.post("/comment_file", Security.authenticateToken, async (req, res) => {
  try {
    ...
    let comment_file_url = await Drive_Service.upload_file(
      req.files.file.name,
      req.files.file.data,
    );
    ...
    await Thoth_DB.post_data(
      "insert into comment_file(comment_id,comment_file_url,comment_file_size,comment_file_name,comment_file_created_date,user_id) values(:comment_id,:comment_file_url,:comment_file_size,:comment_file_name,:comment_file_created_date,:user_id)",
      {
        comment_id,
        comment_file_url,
        comment_file_size,
        comment_file_name,
        comment_file_created_date,
        user_id,
      },
    );
  ...
  async (req, res) => {
    try {
      ...
      for (let i = 0; i <= comment_files.length - 1; i++) {
        comment_files[i].comment_file_url =
          `${Consts.stream_url}${comment_files[i].comment_file_url}`;
      }
