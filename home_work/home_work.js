...
router.post("/home_work", Security.authenticateToken, async (req, res) => {
  try {
    ...
    let home_work = await Thoth_DB.post_data(
      "insert into home_work(rich_text,home_work_description,home_work_total_point,home_work_created_date,home_work_duration_in_days,room_id,user_id) values(:rich_text,:home_work_description,:home_work_total_point,:home_work_created_date,:home_work_duration_in_days,:room_id,:user_id)",
      {
        rich_text,
        home_work_description,
        home_work_total_point,
    ...
    if (rooms.length > 0) {
      ...
      for (let item of members) {
        ...
        if (user_id != owner_user_id) {
          ...
          let message = `Cher ${user_data.user_fullname}, Un nouveau Devoir a ete donnee  dans votre salle ${room_name}. Veuillez vous connecter pour le faire!`;
...
router.patch("/home-work-correction/:home_work_id",Security.authenticateToken,async(req,res)=>{
  try {
    let home_work_id = parseInt(req.params.home_work_id);
    let correction_rich_text = req.body.correction_rich_text;
    await Thoth_DB.update_data("update home_work set correction_rich_text=:correction_rich_text where home_work_id=:home_work_id",{
      correction_rich_text,home_work_id
    })
    res.sendStatus(200);
  } catch (error) {
    serverError(error,res);
  ...
  async (req, res) => {
    try {
      ...
      await Thoth_DB.update_data(
        "update home_work set rich_text = :rich_text where home_work_id=:home_work_id and user_id=:user_id",
        { rich_text, home_work_id, user_id },
      );
