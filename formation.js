...
function validateFormation({ formation, annee_debut, annee_fin, lieu }) {
  if (typeof formation !== "string" || typeof lieu !== "string") {
    return "Invalid data.";
  }
  ...
  if (formation.length > 150 || lieu.length > 150) {
...
router.post("/formation", [Security.authenticateToken], async (req, res) => {
  try {
    ...
    let { formation, annee_debut, annee_fin, lieu, diplome_url } = req.body;
  ...
  async (req, res) => {
    try {
      ...
      let data = await DB.get_data(
        "select diplome_url from formation where formation_id=:formation_id",
        { formation_id },
      );
      if (data[0].diplome_url != diplome_url) {
        if (data[0].diplome_url != null) {
          HelperFile.deleteFileFromServer(data[0].diplome_url);
        }
      }
