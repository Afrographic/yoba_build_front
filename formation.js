...
router.post("/formation", [Security.authenticateToken], async (req, res) => {
  try {
    ...
    let { formation, annee_debut, annee_fin, lieu } = req.body;
    let diplome_url = "";

    if (req.files != undefined) {
      let file = req.files["doc"];
      file.mv(`./public/backend_files/${file.name}`);
      diplome_url = `${Consts.backend_host}/backend_files/${file.name}`;
    }
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      let data = await DB.get_data("select diplome_url from formation where formation_id=:formation_id",{
        formation_id
      })

      let diplome_url = data[0].diplome_url;
