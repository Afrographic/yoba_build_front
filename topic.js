...
router.post("/topic", Security.authenticateToken, async (req, res) => {
  try {
    ...
    let { room_id, topic_content } = req.body;
    let fields_Ok = Validator.validateFields({ room_id, topic_content }, res);
    if (!fields_Ok) {
      return;
    }
    ...
    topic_content = HelperFunction.Ucase(topic_content);
    let chat_id = await HelperFunction.create_chat_instance();

    let data = await Thoth_DB.post_data(
      "insert into topic(topic_content,room_id,user_id,chat_id,topic_created_date) values(:topic_content,:room_id,:user_id,:chat_id,:topic_created_date)",
      { topic_content, room_id, user_id, chat_id, topic_created_date },
    );
    ...
    if (rooms.length > 0) {
      ...
      for (let item of members) {
        ...
        if (user_id != owner_user_id) {
          ...
          let message = `Cher ${user_data.user_fullname}, Un nouveau sujet a ete cree dans le  forum de votre  salle ${room_name} - << ${topic_content}>>`;
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      let { topic_content } = req.body;
      let field_ok = Validator.validateFields({ topic_content }, res);
      if (!field_ok) {
        return;
      }
      ...
      topic_content = HelperFunction.Ucase(topic_content);
      await Thoth_DB.update_data(
        "update topic set topic_content=:topic_content where topic_id=:topic_id and user_id=:user_id",
        { topic_content, topic_id, user_id },
      );
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      let topics = await Thoth_DB.get_data(
        `select user_fullname,user_school,user_avatar,user.user_id,topic.* from topic,user where topic.user_id = user.user_id and topic_content like '%${token}%' and topic.room_id = ${room_id} order by topic_id `,
        {},
      );
