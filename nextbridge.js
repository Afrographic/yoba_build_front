...
router.get("/requettes_nextbrige",[Security.authenticateToken,Security.is_admin],async(req,res)=>{
    try {
        let reqs = await DB.get_data("select * from nextbridge where active = 0");
        for(let i = 0 ; i<=reqs.length-1;i++){
            let nextbridge_id = reqs[i].nextbridge_id;
            let docs = await DB.get_data("select * from nextbridge_docs where nextbridge_id=:nextbridge_id ",{nextbridge_id});
            reqs[i].docs = docs;
            reqs[i].adress = await User_Controller.get_address(reqs[i].user_id);
...
router.get("/nextbridge_agents",Security.authenticateToken,async(req,res)=>{
    try {
        ...
        for(let i = 0 ; i<=agents.length-1;i++){
            let nextbridge_id = agents[i].nextbridge_id;
            let docs = await DB.get_data("select * from nextbridge_docs where nextbridge_id=:nextbridge_id ",{nextbridge_id});
            agents[i].docs = docs;
            agents[i].adress = await User_Controller.get_address(agents[i].user_id);
