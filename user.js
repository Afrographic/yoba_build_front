...
router.post("/create_account", async (req, res) => {
  try {
    ...
    User_Controller.give_money_influencer_for_affiliation(user_id, 100);
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      let msg = `L'utilisateur ${user_data.user_full_name} souhaite activer son compte, veuillez vous connecter pour effectuer l'operation!`;
      for (let item of admins) {
        let admin_data = await User_Controller.get_basic_info(item.user_id);
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      Real_Time.socket.to(`user-${user_id}`).emit("account_activated", {
        message: "Runner",
      });
      ...
      Notification_Controller.new_notif(user_id, message_to_send);
...
//get user location
router.get("/user-location/:id",Security.authenticateToken,async(req,res)=>{
  try {
    let user_id = parseInt(req.params.id);
    let data = await DB.get_data("select geo_lat,geo_long from user where user_id=:user_id ",{user_id})
    res.send(data[0]);
  } catch (error) {
    serverError(error,res);
  }
