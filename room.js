...
router.post("/room", Security.authenticateToken, async (req, res) => {
  try {
    ...
    let room = await Thoth_DB.post_data(
      "insert into room(user_id,room_name,room_avatar,room_created_date,code) values(:user_id,:room_name,:room_avatar,:room_created_date,:code)",
      {
        user_id,
        room_name,
        room_avatar,
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (balance < frais) {
        return res.status(401).send({ solde_min: frais });
      }
      ...
      if (data.length == 0) {
        await Thoth_DB.post_data(
          "insert into user_room(user_id,room_id,user_room_created_date) values(:user_id,:room_id,:user_room_created_date)",
          { user_id, room_id, user_room_created_date },
        );
      }
      res.send({ room_name });
    } catch (error) {
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      if (admins.length == 0) return res.status(401).send("Vous n'etes pas admin de cette salle!");
      let chapters = await Thoth_DB.get_data(
        "select * from chapter where chapter_id = :chapter_id and room_id=:room_id",
        { chapter_id, room_id },
      );
      if (chapters.length == 0) return res.status(401).send("Ce chapitre n'existe pas!");
      //Delete associated cours
      await Thoth_DB.delete_data(
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      if (admins.length == 0) return res.sendStatus(401);
      await Thoth_DB.update_data("update subject set subject_name=:subject_name where subject_id=:subject_id and room_id=:room_id",{
        subject_id,room_id,subject_name
      })
      res.sendStatus(200);
    } catch (error) {
      serverError(error, res);
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      if (admins.length == 0) return res.sendStatus(401);
      await Thoth_DB.update_data("update chapter set chapter_name=:chapter_name where chapter_id=:chapter_id and room_id=:room_id",{
        chapter_id,room_id,chapter_name
      })
      res.sendStatus(200);
    } catch (error) {
      serverError(error, res);
