...
router.post("/comment_voice", Security.authenticateToken, async (req, res) => {
  try {
    ...
    if (
      !Validator.validateFields({ comment_id, comment_voice_duration }, res)
    ) {
      return;
    }
    ...
    let comment_voice_url = await Drive_Service.upload_file(
      req.files.file.name,
      req.files.file.data,
    );

    await Thoth_DB.post_data(
      "insert into comment_voice(comment_id,comment_voice_url,comment_voice_duration,comment_voice_created_date,user_id) values(:comment_id,:comment_voice_url,:comment_voice_duration,:comment_voice_created_date,:user_id)",
      {
        comment_id,
        comment_voice_url,
        comment_voice_duration,
    ...
    CommentService.notifyAllConcernedPeople(
      user_id,
      comments[0].chat_id,
      "Message vocale",
    );
  ...
  async (req, res) => {
    try {
      ...
      for (let i = 0; i <= comment_voices.length - 1; i++) {
        comment_voices[i].comment_voice_url =
          `${Consts.stream_url}${comment_voices[i].comment_voice_url}`;
      }
...
router.delete(
  "/comment_voice/:comment_voice_id",
  Security.authenticateToken,
  async (req, res) => {
    try {
      let user_id = req.body.user_data.user_id;
