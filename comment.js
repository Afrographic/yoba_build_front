  ...
  try {
    ...
    if (parseInt(comment_reply_id) == 0) {
      comment = await Thoth_DB.post_data(
        "insert into comment(comment_contenu,comment_created_date,chat_id,user_id) values(:comment_contenu,:comment_created_date,:chat_id,:user_id)",
        {
          comment_contenu,
    ...
    if (parseInt(comment_reply_id) > 0) {
      comment = await Thoth_DB.post_data(
        "insert into comment(comment_contenu,comment_created_date,comment_reply_id,chat_id,user_id) values(:comment_contenu,:comment_created_date,:comment_reply_id,:chat_id,:user_id)",
        {
          comment_contenu,
          comment_created_date,
          comment_reply_id,
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      comments[0].elapsed_time = HelperFunction.get_elapsed_time(
        comments[0].comment_created_date,
      );
      comments[0].user_avatar = `${Consts.stream_url}${comments[0].user_avatar}`;
      res.send(comments[0]);
    } catch (error) {
      serverError(error, res);
    }
