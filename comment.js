...
router.post("/comment", Security.authenticateToken, async (req, res) => {
  try {
    ...
    notifyAllConcernedPeople(user_id, chat_id, comment_contenu);
...
async function notifyAllConcernedPeople(sender_id, chat_id, comment_contenu) {
  let sender_data = await User_Controller.get_user_data(sender_id);
  //Notifier tous les utilisateurs de la salle
  let rooms = await Thoth_DB.get_data(
    "select * from room where chat_id=:chat_id",
    { chat_id },
  );
  if (rooms.length > 0) {
    ...
    for (let i = 0; i <= members.length - 1; i++) {
      let user_id = members[i].user_id;
      if (user_id == sender_id) continue;
      let user_data = await User_Controller.get_user_data(user_id);

      PushService.new(
        user_id,
        `Nouveau Message  de ${sender_data.user_fullname} dans ${room_name} · ${comment_contenu}`,
      );
    }
  }
}
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      comments[0] = await getCommentMetaData(comments[0]);
      res.send(comments[0]);
    } catch (error) {
      serverError(error, res);
    }
  ...
  async (req, res) => {
    try {
      ...
      for (let i = 0; i <= comments.length - 1; i++) {
        comments[i] = await getCommentMetaData(comments[i]);
      }
...
async function getCommentMetaData(comment) {
  comment.elapsed_time = HelperFunction.get_elapsed_time(
  ...
  //get comment voices
  let comment_voices = await Thoth_DB.get_data(
    "select * from comment_voice where comment_id=:comment_id",
    { comment_id },
  );
  for (let g = 0; g <= comment_voices.length - 1; g++) {
    comment_voices[g].comment_voice_url =
      `${Consts.stream_url}${comment_voices[g].comment_voice_url}`;
  }
  comment.comment_voices = comment_voices;

  //get comment images
  let images = [];
  let comment_images = await Thoth_DB.get_data(
    "select comment_image_url from comment_image where comment_id=:comment_id",
    { comment_id },
  );
  for (const image of comment_images) {
    images.push(`${Consts.stream_url}${image.comment_image_url}`);
  }
  comment.comment_images = images;

  //get comment files
  let comment_files = await Thoth_DB.get_data(
    "select * from comment_file where comment_id=:comment_id",
    { comment_id },
  );
  for (let k = 0; k <= comment_files.length - 1; k++) {
    comment_files[k].comment_file_url =
      `${Consts.stream_url}${comment_files[k].comment_file_url}`;
  }

  comment.files = comment_files;

  //get reply comment
  if (comment.comment_reply_id != null) {
    let comment_reply_id = comment.comment_reply_id;
    let comments = await Thoth_DB.get_data(
      "select user_avatar,user_fullname,comment.* from comment,user where comment_id=:comment_reply_id and comment.user_id = user.user_id",
      { comment_reply_id },
    );
    if (comments.length == 0) {
      return null;
    ...
    comments[0] = await getCommentMetaData(comments[0]);
    comment.reply_comment = comments[0];
  }
