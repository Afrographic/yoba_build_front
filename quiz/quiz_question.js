...
router.post("/question", Security.authenticateToken, async (req, res) => {
  try {
    ...
    let question_image = "";
    let { question_text, quiz_id, props } = req.body;

    if (!Validator.validateFields({ question_text, quiz_id, props }, res)) {
      return;
    }
    ...
    let question = await Thoth_DB.post_data(
      "insert into question(question_text,question_created_date,question_image,quiz_id,user_id) values(:question_text,:question_created_date,:question_image,:quiz_id,:user_id)",
      {
        question_text,
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      if (!Validator.validateFields({ question_text }, res)) {
        return;
      }
      ...
      await Thoth_DB.update_data(
        "update question set question_text=:question_text where question_id=:question_id",
