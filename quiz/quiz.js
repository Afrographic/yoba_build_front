...
const {
  Chapter_Controller,
} = require("../../controllers/chapter_controller.js");
const {
  Subject_Controller,
} = require("../../controllers/subject_controller.js");
...
router.post("/quiz", Security.authenticateToken, async (req, res) => {
  try {
    ...
    let fields_ok = Validator.validateFields(
      {  quiz_duration, level_id, subject_id, chapter_id },
      res,
    );
    ...
    let quiz = await Thoth_DB.post_data(
      "insert into quiz(rich_text,room_id,quiz_consigne,quiz_duration,quiz_image,quiz_created_date,user_id,level_id,subject_id,chapter_id) values(:rich_text,:room_id,:quiz_consigne,:quiz_duration,:quiz_image,:quiz_created_date,:user_id,:level_id,:subject_id,:chapter_id)",
      {
        rich_text,
        room_id,
        quiz_consigne,
    ...
    if (rooms.length > 0) {
      ...
      for (let item of members) {
        ...
        if (user_id != owner_user_id) {
          ...
          let message = `Cher ${user_data.user_fullname}, Un nouveau quiz a ete ajoute sur << ${level_name} ${subject_name} ${chapter_name} >> dans votre salle ${room_name}. Veuillez vous connecter pour en profiter!`;
      ...
      for (let i = 0; i <= followers.length - 1; i++) {
        ...
        let message = `Cher ${user_data.user_fullname}, Un nouveau quiz a ete ajoute << ${level_name} ${subject_name} ${chapter_name} >> par M.${owner_data.user_fullname}. Veuillez vous connecter pour en profiter!`;
        NotifEngine.new(user_id, message);
      }
    }
    res.send({ quiz_id });
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      await Thoth_DB.update_data(
        "update quiz set rich_text=:rich_text where quiz_id=:quiz_id and user_id=:user_id",
        { rich_text, quiz_id,user_id },
      );
...
//get quiz matiere
router.get(
  "/quiz-matiere/:subject_id",
  Security.authenticateToken,
  async (req, res) => {
    try {
      let subject_id = parseInt(req.params.subject_id);
      let data = await Thoth_DB.get_data(
        "select * from quiz where subject_id=:subject_id  order by quiz_id DESC",
        { subject_id },
      );
...
//get quiz chapitre
router.get(
  "/quiz-chapitre/:chapitre_id",
  Security.authenticateToken,
  async (req, res) => {
    try {
      let chapitre_id = parseInt(req.params.chapitre_id);
      let data = await Thoth_DB.get_data(
        "select * from quiz where chapter_id=:chapitre_id  order by quiz_id DESC",
        { chapitre_id },
      );
...
//get chapter list for quiz
router.get(
  "/chapter-list/room/:room_id",
  Security.authenticateToken,
  async (req, res) => {
    try {
      let room_id = parseInt(req.params.room_id);
      let subjects = await Thoth_DB.get_data(
        "select * from subject where room_id=:room_id order by subject_id DESC",
        { room_id },
      );

      for (let i = 0; i < subjects.length; i++) {
        let subject = subjects[i];
        subjects[i].total_quiz = await Subject_Controller.count_quiz(
          subject.subject_id,
        );
        subjects[i].total_cours = await Subject_Controller.count_cours(
          subject.subject_id,
        );
        subjects[i].total_flascard = await Subject_Controller.count_flashcard(
          subject.subject_id,
        );
        let chapters = await Thoth_DB.get_data(
          "select * from chapter where subject_id=:subject_id order by chapter_id DESC",
          { subject_id: subject.subject_id },
        );
        subjects[i].chapters = await Chapter_Controller.get_meta_data(chapters);
      }
